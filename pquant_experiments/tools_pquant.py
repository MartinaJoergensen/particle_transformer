...
def train_classification(
    ...
    with tqdm.tqdm(train_loader) as tq:
        for X, y, _ in tq:
            ...
            inputs[2] = inputs[2] / 2048.0  # normalize pf_vectors GeV → ~[-1,1]
            #print([torch.max(torch.abs(e)) for e in inputs])
            ...
            with torch.cuda.amp.autocast(enabled=grad_scaler is not None):
                ...
                losses = get_model_losses(model, torch.tensor(0.).to("cuda"))
                #print(losses)
                loss += losses
            ...
            if wandb.run is not None:
                wandb.log({
                    "train/loss": loss,
                    "train/acc": correct / num_examples,
                    "train/pruning_loss": losses.item(),
                })
    ...
    if wandb.run is not None:
        wandb.log({
            ...
            "train/acc_epoch": total_correct / count,
            "epoch": epoch,
        })
...
def evaluate_classification(model, test_loader, dev, epoch, for_training=True, loss_func=None, steps_per_epoch=None,
    ...
    # define tb_mode first, before any logging
    tb_mode = 'eval' if for_training else 'test'

    # wandb
    if wandb.run is not None:
        wandb.log({
            f"{tb_mode}/loss_epoch": total_loss / count,
            f"{tb_mode}/acc_epoch": total_correct / count,
            "epoch": epoch,
        })

    # if tb_helper:
    #     tb_helper.write_scalars([
    #         (f"Loss/{tb_mode} (epoch)", total_loss / count, epoch),
    ...
    if wandb.run is not None and for_training:
        wandb.log({"eval/auc_epoch": float(metric_results.get('roc_auc_score', 0)), "epoch": epoch})
    if for_training:
