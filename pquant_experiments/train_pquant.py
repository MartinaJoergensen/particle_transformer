...
def get_model_data_loss(config: PQConfig, device: str):
    with open(f"configs/particle_transformer.yaml", "r") as f:
...
def main(config: PQConfig):
    ...
    global output_dir
    output_dir = "/eos/home-m/majorgen/particle_transformer/pquant_experiments/results/pquant_dst_run7"
    ...
    optimizer = torch.optim.Adam(
            ...
            lr=particle_transformer_config.start_lr,
        )
    scheduler = torch.optim.lr_scheduler.MultiStepLR(
        optimizer, milestones=[24,29,32], gamma=0.1
    )
...
if __name__ == "__main__":
    config = dst_config()
    config.training_parameters.epochs = 35
    ...
    config.quantization_parameters.default_data_integer_bits = 11.
    config.quantization_parameters.default_data_fractional_bits = 8.
    main(config)