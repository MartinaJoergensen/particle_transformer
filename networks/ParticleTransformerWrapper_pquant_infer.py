...
class ParticleTransformerWrapper(torch.nn.Module):
    ...
    def load_state_dict(self, state_dict, strict=True):
        ...
        # pquant version mismatch: final_compression_done may be a plain Python bool
        # (PQConv1d, PQBatchNorm) or an nn.Parameter (PQDense, Quantizer). Neither
        # transfers correctly across versions via load_state_dict. Force all True so
        # the weight property returns _weight directly (already-compressed values).
